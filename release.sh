...
if [[ `go version |grep -c "go1.15.15"` == '0' ]]; then
  ...
  wget -c https://github.com/cs-star/MyUrls/releases/download/v1.10/go1.15.15.linux-${ARCH_PRINT2}.tar.gz -O /root/go1.15.15.linux-${ARCH_PRINT2}.tar.gz
...
rm -rf /root/MyUrls
git clone https://github.com/cs-star/MyUrls /root/MyUrls
if [[ $? -ne 0 ]];then
